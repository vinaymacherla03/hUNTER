...
import { Compass, Target, Zap, ChevronRight, Award, Lightbulb, RefreshCw, TrendingUp, ExternalLink, Loader2, BookOpen, AlertCircle, Map } from 'lucide-react';
                ...
                {isLoading ? (
                    <AIProcessingState 
                        ...
                        messages={[
                            ...
                            "Formulating positioning statement...",
                            "Building 3-step roadmap...",
                    ...
                    >
                        ...
                        <div className="p-10 bg-gradient-to-br from-emerald-600 via-emerald-600 to-emerald-600 rounded-[2rem] text-white shadow-xl shadow-emerald-500/20 relative overflow-hidden border border-emerald-500/30">
                            ...
                            <div className="lg:col-span-2 space-y-6 bg-white p-8 rounded-[2rem] border border-slate-200/50 shadow-sm">
                                ...
                                <div className="space-y-6 relative before:absolute before:inset-0 before:ml-8 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
                                    {strategy.roadmap.map((item, i) => (
                                        <div key={`roadmap-${i}-${item.step}`} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        {strategy.skillGaps.map((skill, i) => (
                                            <button 
                                                key={`skill-gap-${i}-${skill}`} 
                                                onClick={() => fetchResources(skill)}
                                                className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all duration-300 text-left ${selectedSkill === skill ? 'bg-emerald-50 border-emerald-500 shadow-sm' : 'bg-slate-50 border-transparent hover:border-slate-200 hover:bg-slate-100'}`}
                                            >
                                                <div className="flex items-center gap-3 text-sm font-bold text-slate-700">
                                                    <div className={`w-2.5 h-2.5 rounded-full ${selectedSkill === skill ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
                                                    {skill}
                                                </div>
                                                <ChevronRight className={`w-4 h-4 text-slate-400 transition-transform duration-300 ${selectedSkill === skill ? 'rotate-90 text-emerald-500' : ''}`} />
                                            </button>
                                        ))}
                                    </div>
                                        ...
                                        {selectedSkill && (
                                            ...
                                            >
                                                <div className="flex items-center justify-between">
                                                    <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Top Resources for {selectedSkill}:</h4>
                                                    {isFetchingResources && <Loader2 className="w-4 h-4 animate-spin text-emerald-500" />}
                                                </div>
                                                
                                                <div className="space-y-3">
                                                    {skillResources.map((res, i) => (
                                                        <a 
                                                            key={`resource-${i}-${res.url}`}
                                                            href={ensureAbsoluteUrl(res.url)}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-2xl hover:border-emerald-400 hover:shadow-md group transition-all duration-300"
                                                        >
                                                            <div className="flex items-center gap-4">
                                                                <div className="p-2 bg-slate-50 rounded-xl group-hover:bg-emerald-50 transition-colors">
                                                                    <BookOpen className="w-4 h-4 text-slate-400 group-hover:text-emerald-500 transition-colors" />
                                                                </div>
                                                                <div>
                                                                    <p className="text-sm font-bold text-slate-800 leading-tight group-hover:text-emerald-700 transition-colors">{res.title}</p>
                        ...
                        {/* Pro Tip */}
                        <div className="flex items-start gap-6 p-8 bg-gradient-to-r from-emerald-50 to-emerald-50 border border-emerald-100 rounded-[2rem] shadow-sm">
