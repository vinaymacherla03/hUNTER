...
import { Users, Send, MessageSquare, Copy, Check, RefreshCw, Lightbulb, ChevronRight, Sparkles } from 'lucide-react';
...
const NetworkingPanel: React.FC<NetworkingPanelProps> = ({ resumeData, jobDescription }) => {
    ...
    const fetchStrategy = async () => {
        ...
        setError(null);
        try {
            const targetRole = jobDescription || resumeData.title || "Target Role";
            const data = await getNetworkingStrategy(resumeData, targetRole);
            setStrategy(data);
        } catch (err) {
    ...
    const copyToClipboard = (text: string, index: number, isQuestion = false) => {
        ...
        <div className="space-y-8">
            ...
            <AnimatePresence mode="wait">
                {isLoading ? (
                    <AIProcessingState 
                        key="loading"
                        title="Crafting Your Outreach"
                        messages={[
                            "Analyzing your profile...",
                            "Identifying target audience...",
                            "Drafting personalized templates...",
                            "Formulating informational questions...",
                            "Compiling networking advice..."
                        ]}
                        icon={Users}
                        color="emerald"
                    />
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    {strategy.templates.map((template, i) => (
                                        <div key={`template-${i}-${template.type}`} className="p-6 bg-white border-2 border-slate-100 rounded-[2rem] hover:border-emerald-500 transition-all group relative">
                            ...
                            <div className="space-y-8">
                                    ...
                                    <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white space-y-6 relative overflow-hidden">
                                        ...
                                        <div className="space-y-4 relative z-10">
                                            {strategy.advice.map((tip, i) => (
                                                <div key={`advice-${i}-${tip.substring(0, 20)}`} className="flex gap-4">
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-lg font-black text-slate-900 uppercase tracking-widest">Informational Interview Questions</h3>
                                    </div>
                                    <div className="space-y-3">
                                        {strategy.interviewQuestions.map((question, i) => (
                                            <div key={`question-${i}-${question.substring(0, 20)}`} className="p-4 bg-white border border-slate-200 rounded-2xl flex items-center justify-between group hover:border-emerald-500 transition-all">
                                                <p className="text-sm text-slate-700 font-medium leading-relaxed pr-4">
                                                    {question}
                                                </p>
                                                <button 
                                                    onClick={() => copyToClipboard(question, i, true)}
                                                    className="p-2 text-slate-400 hover:text-emerald-600 transition-colors shrink-0"
                                                >
                                                    {copiedQuestionIndex === i ? <Check className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4" />}
                                                </button>
                                            </div>
                                        ))}
                                    </div>
