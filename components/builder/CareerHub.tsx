...
import { TrendingUp, Compass, Building, Sparkles, Zap, ChevronRight, Users, DollarSign, Award, ScanFace, Ghost, Target, Search, ArrowRightLeft, Briefcase, FileText, MessageSquare, Linkedin, Layout } from 'lucide-react';
...
import LinkedInOptimizerPanel from './LinkedInOptimizerPanel';
import CommunicationHubPanel from './CommunicationHubPanel';
import InterviewCheatSheetPanel from './InterviewCheatSheetPanel';
...
type HubTab = 'dashboard' | 'search' | 'automation' | 'tracker' | 'match' | 'gap' | 'cover' | 'interview' | 'cheat' | 'linkedin' | 'comm' | 'trends' | 'strategy' | 'research' | 'networking' | 'salary' | 'scorecard' | 'vibe' | 'ghost' | 'pivot' | 'roadmap' | 'internships';

const CareerHub: React.FC<CareerHubProps> = ({ resumeData, jobDescription, onJobDescriptionChange }) => {
...
{activeTab === 'gap' && <SkillGapPanel resumeData={resumeData} jobDescription={jobDescription || ''} />}
                    {activeTab === 'cover' && <CoverLetterPanel resumeData={resumeData} jobDescription={jobDescription || ''} />}
                    {activeTab === 'interview' && <MockInterviewPanel resumeData={resumeData} jobDescription={jobDescription || ''} />}
                    {activeTab === 'linkedin' && <LinkedInOptimizerPanel resumeData={resumeData} />}
                    {activeTab === 'comm' && <CommunicationHubPanel resumeData={resumeData} jobDescription={jobDescription} />}
                    {activeTab === 'cheat' && <InterviewCheatSheetPanel resumeData={resumeData} jobDescription={jobDescription} />}
                    ...
                    {activeTab === 'networking' && <NetworkingPanel resumeData={resumeData} jobDescription={jobDescription} />}
