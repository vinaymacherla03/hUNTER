...
const ModernSidebarRight: React.FC<TemplateProps> = ({ data, sectionOrder, customization, sectionVisibility, onDataChange }) => {
          ...
          {(exp, i) => (
            <div key={exp.id} className="mb-6 last:mb-0">
              ...
              <div className="mb-3 flex gap-2 items-center">
                <EditableField path={`experience[${i}].company`} value={exp.company} onChange={onDataChange} className="font-black text-indigo-600 text-xs uppercase tracking-widest" />
            ...
            {(proj, i) => (
              <div key={proj.id} className="mb-4 last:mb-0">
                <div className="flex justify-between items-baseline mb-1">
                    ...
                    <EditableField path={`projects[${i}].role`} value={proj.role} onChange={onDataChange} className="text-indigo-500 text-[10px] font-black" />
                </div>
    ...
    <div id="resume-content" className="bg-white font-sans flex min-h-[11in] w-full overflow-hidden">
      ...
      <main className="flex-1 p-12 bg-white">
        <header className="mb-12">
            <EditableField as="h1" path="fullName" value={data.fullName} onChange={onDataChange} className="text-5xl font-black leading-tight tracking-tighter text-slate-900 mb-2" />
            <EditableField as="p" path="title" value={data.title} onChange={onDataChange} className="text-indigo-600 font-black text-[10px] uppercase tracking-[0.3em]" />
        </header>
        {sectionOrder.filter(k => !['skills', 'education'].includes(k) && sectionVisibility[k]).map(k => mainSections[k])}
      </main>

      {/* Sidebar */}
      <aside className="w-1/3 bg-[#1e1b4b] p-8 text-white flex flex-col">
...
export default ModernSidebarRight;
