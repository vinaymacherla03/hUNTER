...
const EnhancvProfessional: React.FC<TemplateProps> = ({ data, sectionOrder, customization, sectionVisibility, onDataChange }) => {
  const marginValue = { compact: '0.4in', normal: '0.6in', spacious: '0.8in' }[customization.margin] || '0.6in';
  
  const sectionTitleStyle: React.CSSProperties = {
    fontSize: `13pt`,
    color: customization.sectionTitleColor || '#111827',
    textTransform: customization.sectionTitleUppercase ? 'uppercase' : 'none',
    letterSpacing: '0.05em',
    marginBottom: '12px',
    marginTop: '24px',
    fontWeight: '800',
    borderBottom: `2px solid ${customization.sectionTitleBorderColor || '#e2e8f0'}`,
    paddingBottom: '4px',
  };
  ...
  const sections: Record<ResumeSectionKey, React.ReactNode> = {
          ...
          {(exp, i) => (
            <div key={exp.id} className="mb-6 last:mb-0">
              <div className="flex justify-between items-baseline mb-1">
                ...
                <EditableField path={`experience[${i}].dates`} value={exp.dates} onChange={onDataChange} className="text-slate-500 text-xs font-semibold uppercase tracking-wider" />
              </div>
          ...
          <EditableList items={data.projects || []} path="projects" onChange={onDataChange} newItem={{ id: '', name: 'Project', role: 'Role', description: ['Detail'] }}>
            {(proj, i) => (
              <div key={proj.id} className="mb-4 last:mb-0">
                <div className="flex justify-between items-baseline mb-1">
                    <EditableField path={`projects[${i}].name`} value={proj.name} onChange={onDataChange} className="font-bold text-slate-900 text-sm uppercase tracking-wide" />
                    <EditableField path={`projects[${i}].role`} value={proj.role} onChange={onDataChange} className="text-blue-600 text-xs font-bold" />
                </div>
                <div className="text-slate-700 text-xs leading-relaxed space-y-1">
                    {proj.description.map((desc, di) => (
                        <EditableField key={di} as="p" path={`projects[${i}].description[${di}]`} value={desc} onChange={onDataChange} className="flex gap-2">
            ...
            {(edu, i) => (
              <div key={edu.id} className="mb-4 last:mb-0">
                ...
                <div className="flex gap-2 text-xs mt-1">
                    <EditableField path={`education[${i}].institution`} value={edu.institution} onChange={onDataChange} className="text-blue-600 font-bold" />
                    <span className="text-slate-300">•</span>
    ...
    skills: (
      <section key="skills" className="mb-8">
        <h2 style={sectionTitleStyle}>Skills & Competencies</h2>
        <div className="grid grid-cols-2 gap-x-8 gap-y-4">
          {data.skills.map((cat, i) => (
            <div key={cat.id} className="flex flex-col">
              <EditableField path={`skills[${i}].name`} value={cat.name} onChange={onDataChange} className="font-bold text-slate-900 text-xs mb-2 uppercase tracking-wider" />
              <div className="flex flex-wrap gap-2">
                {cat.skills.map((s, si) => (
                    <span key={si} className="px-2 py-1 bg-slate-100 text-slate-700 rounded text-[10px] font-bold uppercase tracking-widest">
                        {s.name}
                    </span>
                ))}
              </div>
    ...
    awards: (
        <section key="awards" className="mb-8">
          <h2 style={sectionTitleStyle}>Awards & Honors</h2>
          <div className="grid grid-cols-2 gap-4">
        ...
        <section key="keywords" className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {data.keywords?.map((kw, i) => (
              <span key={i} className="px-3 py-1 border border-slate-200 text-slate-600 rounded-full text-[10px] font-bold uppercase tracking-widest">
                {kw}
              </span>
            ))}
          </div>
  ...
  return (
    <div id="resume-content" style={{ padding: marginValue }} className="bg-white font-sans text-slate-900">
      <header className="mb-10 flex flex-col items-center text-center">
        <EditableField as="h1" path="fullName" value={data.fullName} onChange={onDataChange} className="text-4xl font-black leading-tight tracking-tighter text-slate-900 mb-2 uppercase" />
        <EditableField as="p" path="title" value={data.title} onChange={onDataChange} className="text-blue-600 font-bold text-sm uppercase tracking-[0.2em] mb-6" />
...
export default EnhancvProfessional;
